...
| 🟡 达标 | 胜率统计显著性 | 每人至少 30 条有效信号（MacKinlay 1997 事件研究法最小样本量，80% 统计功效），否则输出"样本不足" | 自动检查：`total_signals < 30 → {"status": "insufficient_data"}` |
| 🟡 达标 | SPY 基准准确性 | SPY 收益计算与 Yahoo Finance 偏差 < 0.5% | 抽 10 个日期交叉验证 `prices.json` 的 SPY close vs Yahoo Finance |
| 🟢 优秀 | 样本量充分 | 每人 ≥ 100 条有效信号（95% 置信区间缩窄到 ±9%），可正式依赖 | 自动检查：`total_signals >= 100` |
| 🟢 优秀 | 多时间窗口一致性 | 7 天胜率与 30 天胜率的 Spearman 相关性 ρ > 0.6（Cohen 1988 中-强相关） | 同一个人，短期和长期判断应正相关 |
