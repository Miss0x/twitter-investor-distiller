...
class PipelineExecuteCard(Card):
    ...
    def get_data(self, **params) -> dict:
        try:
            ...
            db.init_db()
            s = db.get_session()
            tasks = s.query(PipelineTask).order_by(PipelineTask.id.desc()).limit(100).all()
            by_type = {}
            for t in tasks:
                by_type.setdefault(t.task_type, []).append({"id": t.id, "status": t.status})
            s.close()
            # 执行中
            from src.pipeline.task_executor import is_running, get_progress
            return {
                "task_types": {k: len(v) for k, v in by_type.items()},
                "pending": sum(1 for t in tasks if t.status == "pending"),
                "failed": sum(1 for t in tasks if t.status == "failed"),
                "running": is_running(),
                "progress": get_progress(),
                "types_with_pending": {k: sum(1 for vv in v if vv["status"] == "pending") for k, v in by_type.items()},
            }
        except Exception:
            return {"task_types": {}, "pending": 0, "failed": 0, "running": False, "progress": None, "types_with_pending": {}}

    def _render_html(self, data: dict) -> str:
        types = data.get("types_with_pending", {})
        running = data.get("running", False)
        status = "执行中..." if running else "空闲"
        dot = "ok" if running else ""
        btns = ""
        for task_type in ["filter", "analyze", "fetch_price", "fetch_crypto", "clean", "portrait"]:
            pend = types.get(task_type, 0)
            if pend > 0:
                btns += f'<button class="btn btn-primary" onclick="runPipeline(\'{task_type}\')" style="margin:2px">{task_type} ({pend})</button>'
        if not btns:
            btns = '<span class="text-secondary">无待处理任务</span>'
        return f'''<div class="card-title">流水线执行</div>
<div class="flex mb-sm"><div class="status-dot {dot}"></div><span>{status}</span></div>
<div style="margin-bottom:8px">{btns}</div>
<div class="flex" style="gap:8px">
  <button class="btn" onclick="runPipeline('seed')" style="font-size:11px">种子任务</button>
  <button class="btn" onclick="runPipeline('filter_scan')" style="font-size:11px">扫描新推文</button>
  <span id="pe_status" class="text-secondary" style="font-size:11px"></span>
</div>
<script>
async function runPipeline(type){{
  var s=document.getElementById("pe_status"); s.innerText=type+"...";
  try{{
    var r=await fetch("/cards/pipeline_execute/action",{{method:"POST",headers:{{"Content-Type":"application/json"}},body:JSON.stringify({{action:type}})}});
    var d=await r.json(); s.innerText=d.ok?"完成":"失败: "+(d.error||"");
