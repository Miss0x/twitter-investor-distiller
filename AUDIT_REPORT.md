...
| 严重性 | 数量 | 立即影响 |
|--------|------|---------|
| P0 崩溃 | 11 | **6 个功能完全不可用** (fetch_user_info、种子任务、Timeline、Daemon 状态、Telegram、角色代入) + 密钥泄露 + 配置冲突 |
| P1 破损 | 22 | 路径遍历、认证缺失、Session 泄漏、性能衰减 |
| P2 隐患 | 26 | 静默错误、XSS 风险、N+1 查询、向量检索无效 |
| P3 优化 | 13 | 代码风格、死代码、日志混乱 |
| **总计** | **72** | |
